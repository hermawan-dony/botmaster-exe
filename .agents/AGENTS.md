...
11. **UI & Startup Preferences**: The main application must support being maximized (`MaximizeBox = True`, `FormBorderStyle = Sizable`). `FrmMain_Load` implements an AutoRun logic (checking/setting `HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run`) if checked by the user in the Settings tab.
12. **Theme Manager Restriction**: NEVER attempt to implement a dynamic `ThemeManager` or loop through controls to override `BackColor` or `ForeColor`. Modifying these dynamically breaks Windows Forms visual styles (causing invisible text, white CheckBoxes, and broken Buttons due to `UseVisualStyleBackColor`). Always rely purely on the original dark/purple colors baked directly into the `.Designer.vb` source files. Do not add any theme selection options to the UI.
