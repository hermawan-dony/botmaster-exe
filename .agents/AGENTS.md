...
11. **UI & Startup Preferences**: The main application must support being maximized (`MaximizeBox = True`, `FormBorderStyle = Sizable`), all listviews and buttons should adhere to the flat dark theme (`ThemeManager.vb` handles `ListView` and forces `FlatStyle.Flat`), and `FrmMain_Load` implements an AutoRun logic (checking/setting `HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run`) if checked by the user in the Settings tab.
